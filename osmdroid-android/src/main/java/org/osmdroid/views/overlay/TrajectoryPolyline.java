 ...
 */
public class TrajectoryPolyline extends OverlayWithIW {
     ...
     */
    public TrajectoryPolyline(MapView mapView) {
        if (mapView != null) {
    ...
    public void draw(final Canvas canvas, final MapView mapView, final boolean shadow) {
        ...
        if (mOriginalPoints.size() > 0 && mIcon != null) {
            Point mPositionPixels = new Point();
            int width = mIcon.getIntrinsicWidth();
            int height = mIcon.getIntrinsicHeight();
            Rect rect = new Rect(0, 0, width, height);
            rect.offset(-(int) (0.5f * width), -(int) (0.5f * height));
            mIcon.setBounds(rect);

            mIcon.setAlpha((int) (1.0f * 255));
            float rotationOnScreen = mapView.getMapOrientation();
            for (IGeoPoint point : mOriginalPoints) {
    ...
    public interface OnClickListener {
        abstract boolean onClick(TrajectoryPolyline polyline, MapView mapView, GeoPoint eventPos);
    }

    /**
     * default behaviour when no click listener is set
     */
    public boolean onClickDefault(TrajectoryPolyline polyline, MapView mapView, GeoPoint eventPos) {
